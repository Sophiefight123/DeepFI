...
def stratified_split(X, y, test_size=0.1, val_size=0.1, random_state=42):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X_train, y_train, test_size=val_relative_size,
...
X_train, X_val, X_test, y_train, y_val, y_test = stratified_split(X, y, test_size=0.1, val_size=0.1, random_state=seed)
...
class FlowAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k_sum = k.sum(dim=2, keepdim=True) + self.eps  # (B, H, 1, d)
        z = 1.0 / (torch.einsum('bhnd,bhkd->bhnk', q, k_sum).squeeze(-1) + self.eps)  # (B, H, N)
        z = z.unsqueeze(-1)  # (B, H, N, 1)

        # Flow attention computation
        kv = torch.einsum('bhnd,bhnc->bhdc', k, v)  # (B, H, d, d)
        attention = torch.einsum('bhnd,bhdc->bhnc', q, kv)  # (B, H, N, d)
...
if has_two_classes:
    metrics['AUC'] = roc_auc_score(test_labels, test_probs.flatten())  # 展平为一维
else:
